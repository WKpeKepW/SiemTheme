    ...
    RulesChangerCallBack(arr, timer = this.Timer){
        ...
        setInterval(()=>{
            arr.forEach((e)=>{
                ...
                {
                    document.querySelectorAll('iframe').forEach(elem=>{
                        for(let i = 0; i < elem.contentWindow.document.getElementsByTagName(e.selector).length; i++)
                            elem.contentWindow.document.getElementsByTagName(e.selector)[i].style[e.element] = e.new
                    })
                }
                
                else if(e.iframe && document.querySelector('iframe')?.contentWindow.document.getElementsByClassName(e.selector)){
                    document.querySelectorAll('iframe').forEach(elem=>{
                        for(let i = 0; i < elem.contentWindow.document.getElementsByClassName(e.selector).length; i++)
                            elem.contentWindow.document.getElementsByClassName(e.selector)[i].style[e.element] = e.new
                    })
                } 
            })
        },timer)
