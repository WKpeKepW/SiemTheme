...
function checkSelector(selector, func){
    let timeOut = 0
    let Interval = setInterval(() =>{
        if($(selector).hasClass(selector.slice(1)) || $(selector).css('background') != undefined) {
...
function checkSelector2(iframe, selector, func){
    let timeOut = 0
    let Interval = setInterval(() =>{
        if($(iframe).contents().find('*').find(selector).hasClass(selector.slice(1)) || $(iframe).contents().find('*').find(selector).css('background')!=undefined) {
...
function checkSelector2Loop(iframe, selector, func, timer = Timer){
    setInterval(() =>{
        if($(iframe).contents().find('*').find(selector).hasClass(selector.slice(1)) || $(iframe).contents().find('*').find(selector).css('background') != undefined) { //изменить условие нахождение 
            func()
        }
    }, timer)
}
