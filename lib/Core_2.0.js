...
class CoreClass{
    ...
    constructor(arr,timer = 100, timeOut = 5000, iframeSheetInitialize = 3){
        this.ArrayRules = arr
        this.Timer = timer
        this.TimeOut = timeOut
    ...
    RulesChanger(arr = this.ArrayRules){
        ...
        checkReturn(()=>{
            //console.log(document.querySelector('iframe').contentWindow.document.styleSheets)
    ...
    NewRule(arrSelectors){
        arrSelectors.forEach((e)=>{
            document.styleSheets[0].insertRule(e.selector)
            checkReturn(()=>{
                if(document.querySelector('iframe')?.styleSheets){
